...
type Tool = 'story' | 'worksheet' | 'explainer' | 'planner' | 'game' | 'audio-overview' | 'deep-dive' | null;
...
export function StudioPanel({ isOpen, toggle }: StudioPanelProps) {
    ...
    const renderTool = () => {
        if (!activeTool) {
            return (
                <StudioDashboard onSelectTool={setActiveTool} />
            );
        }
        ...
        switch (activeTool) {
            ...
            default:
                ...
                setActiveTool(null);
                return <StudioDashboard onSelectTool={setActiveTool} />;
        }
    ...
    return (
        <aside className="hidden md:flex flex-col w-[350px] md:w-[400px] lg:w-[450px] border-l border-border bg-card/40">
            <div className="p-4 border-b border-border flex items-center justify-between h-16 shrink-0">
                ...
                {toggle && (
                    <Button variant="ghost" size="icon" onClick={toggle} title="Collapse studio panel">
